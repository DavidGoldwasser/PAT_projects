  ...
  def run(model, runner, user_arguments)
    ...
    if args['add_space_type_loads']
      ...
      model.getSpaceTypes.each do |space_type|
        ...
        test = space_type.apply_internal_loads(args['template'],true,true,true,true,true,false)
        if test == false
    ...
    if args['add_constructions']
      ...
      ground_temp_vals = model.find_object($os_standards['ground_temperatures'], 'template' => args[:template], 'climate_zone' => climate_zone, 'building_type' => primary_bldg_type)
      if ground_temp_vals && ground_temp_vals['jan']
        ...
        runner.registerWarning('Could not find ground temperatures; will use generic temperatures, which will skew results.')
        ground_temp = model.getSiteGroundTemperatureBuildingSurface
    ...
    if args['template'] == '90.1-2010'
      if model.runSizingRun("#{Dir.pwd}/SRvt") == false
        return false
      end
    end
    model.add_daylighting_controls(args['template'])
    ...
    if args['add_hvac']
      ...
      case args['system_type']
        when 'Inferred'
          ...
          sys_groups = model.group_zones_by_type(OpenStudio.convert(20_000, 'ft^2', 'm^2').get)
          ...
          sys_groups.each do |sys_group|
            ...
            story_zone_lists = model.group_zones_by_story(sys_group['zones'])
            ...
            story_zone_lists.each do |story_group|
              ...
              pri_sec_zone_lists = model.differentiate_primary_secondary_thermal_zones(story_group)
              # Add the primary system to the primary zones
              model.add_hvac_system(args['template'], sys_type, central_htg_fuel, zone_htg_fuel, clg_fuel, pri_sec_zone_lists['primary'])
              # Add the secondary system to the secondary zones (if any)
              if pri_sec_zone_lists['secondary'].size > 0
                model.add_hvac_system(args['template'], sec_sys_type, central_htg_fuel, zone_htg_fuel, clg_fuel, pri_sec_zone_lists['secondary'])
              end
            end
