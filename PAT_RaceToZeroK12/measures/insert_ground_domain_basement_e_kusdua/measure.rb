...
# start the measure
class InsertGroundDomainBasementEKusdua < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  def description
    return "Measure to insert the EnergyPlus Site:GroundDomain:Basement object into the model."
  end

  # human readable description of modeling approach
  def modeler_description
    return "In EnergyPlus 8.3 hitting issues with time series data when using finite difference. This will add code to loook pre-calculated inputs for "
  end
  ...
  def arguments(workspace)
    ...
    choices << "Static 2016" # used for initial fesability study across climate zones
    choices << "ClimateZone Specific" # climate zone specific lookup added in January 2017
    choices << "No Vertical Insulation" # run slab calcs but don't add any insulation
    insul_target = OpenStudio::Ruleset::OSArgument::makeChoiceArgument("insul_target", choices)
    insul_target.setDisplayName("Insulation_Target")
    insul_target.setDefaultValue("ClimateZone Specific")
    args << insul_target
  ...
  def run(workspace, runner, user_arguments)
    ...
    # get arguments
    insul_target = runner.getStringArgumentValue("insul_target",user_arguments)
    ...
    # todo - add epws for climate zone 0
    ...
    # Site:GroundDomain:Slab doesn't seem to like n-mass material
=begin

    # Adding slab insulation
    string ="
    Material:NoMass,
      VERTICAL SLAB INSULATION NOMASS,                    !- Name
      Rough,                   !- Roughness
      #{rvins},                !- Thermal Resistance {m2-K/W}
...
InsertGroundDomainBasementEKusdua.new.registerWithApplication
