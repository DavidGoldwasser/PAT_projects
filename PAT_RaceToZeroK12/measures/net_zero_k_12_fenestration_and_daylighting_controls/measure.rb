...
#start the measure
class NetZeroK12FenestrationAndDaylightingControls < OpenStudio::Ruleset::ModelUserScript
  
  #define the name that a user will see, this method may be deprecated as
  #the display name in PAT comes from the name field in measure.xml
  def name
    return "Net Zero K12 - Fenestration and Daylighting Controls"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    if target == "AEDG K-12 - ZEDG 2017" || target == "AEDG K-12 - ZEDG 2017 with Daylighting"
    ...
    model.getThermalZones.each do |thermalZone|
      ...
      thermalZone.spaces.each do |space|
        ...
        space.surfaces.each do |surface|
          ...
          if surface.surfaceType == "Wall"
            ...
            if addDaylightGlazing == false || target == "AEDG K-12 - ZEDG 2017"
              daylightWwrS_s = 0.0
              daylightWwrN_s = 0.0
            else
              ...
              vector.each do |subSurface|
                if subSurface.construction.get == viewConstructionSouth
                  ...
                  if target == "AEDG K-12 - ZEDG 2017"
                    daylightSouthHash[vector.last] = vector.last.grossArea # push the daylight window to array for use later on (12/29 copied to view window)
                  end
      ...
      zoneSpacesHash.each do |space, hash|
        ...
        if hash["skylight"]
          ...
          # 12/29 cut space height in half because of removed dalyighting glazing.
          # this will move sensors closer to wall and lower fraction of zone they control
          if target == "AEDG K-12 - ZEDG 2017"
            hash['spaceHeight'] = hash['spaceHeight']/2.0
          end
      ...
      if not spaceToHookToZone.nil?
        ...
        # 12/29 cut this in half along with wall height so no high daylight windows
        if target == "AEDG K-12 - ZEDG 2017"
          zoneDaylightFraction = 0.5*zoneDaylightWallArea/zoneFloorArea
        else
    ...
    aedgTipsLong = '' # hiding AEDG tips on net zero copy of measure
    if not aedgTipsLong
...
NetZeroK12FenestrationAndDaylightingControls.new.registerWithApplication