...
#start the measure
class NetZeroK12EnvelopeInfiltration < OpenStudio::Ruleset::ModelUserScript
  ...
  def name
    return " Net Zero K12 Envelope Infiltration"
  end
  ...
  def run(model, runner, user_arguments)
    ...
    targetFlowPerExteriorArea = 0.0001905  #0.0375 cfm/ft^2 # todo - confirm proper value for this
    ...
    space_infiltration_objects = model.getSpaceInfiltrationDesignFlowRates
    if space_infiltration_objects.size > 0
      runner.registerFinalCondition("The final model contains #{space_infiltration_objects.size} space infiltration objects. Cost was increased by $#{OpenStudio::toNeatString(envelopeImprovementTotalCost,2,true)} for envelope infiltration. #{aedgTipsLong}")
    else
      runner.registerFinalCondition("The final model does not contain any space infiltration objects. Cost was increased by $#{OpenStudio::toNeatString(envelopeImprovementTotalCost,2,true)} for envelope infiltration. #{aedgTipsLong}")
    end
...
NetZeroK12EnvelopeInfiltration.new.registerWithApplication