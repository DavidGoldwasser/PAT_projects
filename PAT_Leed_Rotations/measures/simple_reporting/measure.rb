...
#start the measure
class SimpleReporting < OpenStudio::Ruleset::ReportingUserScript

  # human readable name
  def name
    return "Simple Reporting"
  end

  # human readable description
  def description
    return "This is templale measure, but with test updated so simulation results are saved after first test, so future test runs are quicker."
  end

  # human readable description of modeling approach
  def modeler_description
    return "If the test model is chagned at all it is important ot throw away the results so they will be re-run"
  end
...
SimpleReporting.new.registerWithApplication
